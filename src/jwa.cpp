...
const boost::bimap<JWA::KeyType::Type, boost::bimaps::unordered_set_of<std::string>> KeyType::lookup = make_bimap<JWA::KeyType::Type, boost::bimaps::unordered_set_of<std::string>>({
    {JWA::KeyType::OCT, "oct"},
    {JWA::KeyType::RSA, "RSA"},
    {JWA::KeyType::EC, "EC"},
});
...
struct JWAImpl {
    ...
    JWAImpl(void *_): doc{*reinterpret_cast<std::shared_ptr<rapidjson::Document>*>(_)} {}
    bool parse() {
...
static inline JWAImpl *impl(void * _) {
    return reinterpret_cast<JWAImpl*>(_);
}
...
const KeyType::Type & JWA::kty() const {
    return impl(_)->kty;
}
