...
const boost::bimap<JWA_EC::Curve::Type,
                   boost::bimaps::unordered_set_of<std::string>>
    CurveImpl::lookup =
        make_bimap<JWA_EC::Curve::Type,
                   boost::bimaps::unordered_set_of<std::string>>({
...
JWA_EC::JWA_EC(const std::string &json)
    : _{new JWA_ECImpl{json}}, jose_{&impl(_)->doc}, valid_{false} {
    init_();
}

JWA_EC::JWA_EC(void *_)
    : _{new JWA_ECImpl{_}}, jose_{&impl(_)->doc}, valid_{false} {
    init_();
}
...
JWA_EC::Curve::Type JWA_EC::crv() const { return impl(_)->crv; }

std::string JWA_EC::x() const { return urlsafe_base64_encode(x_raw()); }
const ustring &JWA_EC::x_raw() const { return impl(_)->x; }

std::string JWA_EC::y() const { return urlsafe_base64_encode(y_raw()); }
const ustring &JWA_EC::y_raw() const { return impl(_)->y; }

std::string JWA_EC::d() const { return urlsafe_base64_encode(d_raw()); }
const ustring &JWA_EC::d_raw() const { return impl(_)->d; }
