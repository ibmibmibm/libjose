...
JWA_OCT::JWA_OCT(const std::string &json)
    : _{new JWA_OCTImpl{json}}, jose_{&impl(_)->doc}, valid_{false} {
    init_();
}

JWA_OCT::JWA_OCT(void *_)
    : _{new JWA_OCTImpl{_}}, jose_{&impl(_)->doc}, valid_{false} {
    init_();
}
...
JWA_OCT::~JWA_OCT() { delete impl(_); }

std::string JWA_OCT::k() const { return urlsafe_base64_encode(k_raw()); }
const ustring &JWA_OCT::k_raw() const { return impl(_)->k; }
