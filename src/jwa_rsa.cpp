...
struct Key {
    ...
    static const boost::bimap<Key::Type,
...
JWA_RSA::JWA_RSA(const std::string &json)
    : _{new JWA_RSAImpl{json}}, jose_{&impl(_)->doc}, valid_{false} {
    init_();
}

JWA_RSA::JWA_RSA(void *_)
    : _{new JWA_RSAImpl{_}}, jose_{&impl(_)->doc}, valid_{false} {
    init_();
}
...
JWA_RSA::~JWA_RSA() { delete impl(_); }

std::string JWA_RSA::to_pem() const {
    if (impl(_)->private_key) {
        return RSAPrivateKey2PEM(n_raw(), e_raw(), d_raw(), p_raw(), q_raw(),
                                 dp_raw(), dq_raw(), qi_raw());
    } else {
        return RSAPublicKey2PEM(n_raw(), e_raw());
    }
}

std::string JWA_RSA::n() const { return urlsafe_base64_encode(n_raw()); }
const ustring &JWA_RSA::n_raw() const { return impl(_)->n; }

std::string JWA_RSA::e() const { return urlsafe_base64_encode(e_raw()); }
const ustring &JWA_RSA::e_raw() const { return impl(_)->e; }

std::string JWA_RSA::d() const { return urlsafe_base64_encode(d_raw()); }
const ustring &JWA_RSA::d_raw() const { return impl(_)->d; }

std::string JWA_RSA::p() const { return urlsafe_base64_encode(p_raw()); }
const ustring &JWA_RSA::p_raw() const { return impl(_)->p; }

std::string JWA_RSA::q() const { return urlsafe_base64_encode(q_raw()); }
const ustring &JWA_RSA::q_raw() const { return impl(_)->q; }

std::string JWA_RSA::dp() const { return urlsafe_base64_encode(dp_raw()); }
const ustring &JWA_RSA::dp_raw() const { return impl(_)->dp; }

std::string JWA_RSA::dq() const { return urlsafe_base64_encode(dq_raw()); }
const ustring &JWA_RSA::dq_raw() const { return impl(_)->dq; }

std::string JWA_RSA::qi() const { return urlsafe_base64_encode(qi_raw()); }
const ustring &JWA_RSA::qi_raw() const { return impl(_)->qi; }
