...
static inline JWKImpl *impl(void *_) { return reinterpret_cast<JWKImpl *>(_); }

} // namespace

JWK::JWK() : _{new JWKImpl}, jwa_{_}, valid_{false} {}

JWK::JWK(const std::string &json)
    : _{new JWKImpl{json}}, jwa_{_}, valid_{false} {
    if (!jwa_) {
        return;
    }
    valid_ = impl(_)->parse();
...
JWK::~JWK() { delete impl(_); }

const std::string &JWK::kid() const { return impl(_)->kid; }
