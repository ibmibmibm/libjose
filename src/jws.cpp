...
const boost::bimap<JWS::Alg::Type, boost::bimaps::unordered_set_of<std::string>> AlgImpl::lookup = make_bimap<JWS::Alg::Type, boost::bimaps::unordered_set_of<std::string>>({
    {JWS::Alg::HS256, "HS256"},
    {JWS::Alg::HS384, "HS384"},
    {JWS::Alg::HS512, "HS512"},
    {JWS::Alg::RS256, "RS256"},
...
static inline bool is_compatibility(JWA::KeyType::Type kty, JWS::Alg::Type alg) {
    switch (kty) {
        default:
            return false;
...
static inline HashFunc::Type get_hash(JWS::Alg::Type alg) {
    switch (alg) {
        case JWS::Alg::HS256: case JWS::Alg::RS256: case JWS::Alg::ES256: return HashFunc::SHA256;
        case JWS::Alg::HS384: case JWS::Alg::RS384: case JWS::Alg::ES384: return HashFunc::SHA384;
        case JWS::Alg::HS512: case JWS::Alg::RS512: case JWS::Alg::ES512: return HashFunc::SHA512;
        default: return HashFunc::NONE;
    }
}
...
struct JWSImpl {
    ...
    JWSImpl(const std::string &header_base64, const std::string &payload_base64, const std::string &signature_base64):
            doc{new rapidjson::Document},
            header{urlsafe_base64_decode(header_base64)},
    ...
    std::string payload_base64() const {return urlsafe_base64_encode(payload);}
    std::string signature_base64() const {return urlsafe_base64_encode(signature);}
    JWSImpl(std::shared_ptr<rapidjson::Document> _): doc{_} {}
    void sync_header() {
        typedef rapidjson::GenericStringBuffer<rapidjson::UTF8<unsigned char>> UStringBuffer;
        UStringBuffer s;
...
static inline JWSImpl *impl(void * _) {
    return reinterpret_cast<JWSImpl*>(_);
}
...
JWS::JWS(const std::string &header, const std::string &payload, const std::string &signature): _{new JWSImpl{header, payload, signature}}, valid_{false}  {
    valid_ = impl(_)->parse();
}
...
std::string JWS::header() const {
    return impl(_)->header_base64();
}

std::string JWS::payload() const {
    return impl(_)->payload_base64();
}

std::string JWS::signature() const {
    return impl(_)->signature_base64();
}
...
void JWS::set_jwk(const JWK &jwk) {
    ...
    header.AddMember(
        ...
        rapidjson::Value(jwk.to_json().c_str(), allocator),
        allocator
    );
    impl(_)->sync_header();
}
